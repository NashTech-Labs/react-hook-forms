...
const getDealValues = (deal: any) => {
  const {
    dealValue: { scopeType, rewardsValue, rewardType, spend },
  } = deal;
  const dealValues: any = {};
  if (scopeType === "PRODUCT") {
    if (rewardType === "$_OFF") {
      dealValues["dollarOff"] = String(
        convertCentsToDollar(rewardsValue?.[0]?.value)
      );
  ...
  dealValues["basketDiscount"] =
    rewardType === "%_OFF"
      ? String(rewardsValue?.[0]?.value)
      : String(convertCentsToDollar(rewardsValue?.[0]?.value));
...
const getMultibuyValues = (deal: any) => {
  const values: any = {};
  const {
    generalDealInfo: { type },
    dealValue: { rewardsValue, rewardType, quantity },
  } = deal;
  if (type === MULTI_BUY_DEAL_TYPE) {
    values["dealCriteria"] = rewardsValue.map(
      ({ value, restrictions }: any) => {
        const criteria: any = {
          get:
            rewardType === "%_OFF_MULTI"
              ? String(value)
              : String(convertCentsToDollar(value)),
        };
        if (restrictions) {
          const {
            quantity: { minimum },
...
const getMchLiamValues = (deal: any) => {
  const mch: Array<string> = [];
  const liam: Array<string> = [];
  const {
    dealValue: { scopeValue },
    exclusion: { product },
  } = deal;
  scopeValue?.forEach(({ value, sub_type }: any) => {
    if (sub_type === "MCH") {
      mch.push(value);
    } else {
      liam.push(value);
...
const convertDealDataToFormData = (deal: any) => {
  ...
  formData["stackingType"] =
    Object.keys(STACKING_TYPES).find(
      (key) => STACKING_TYPES[key] === stacking_type
    ) || "";
  ...
  formData["dealApplyType"] =
    priceApplicability === null ? "all" : "regular_priced_only";
