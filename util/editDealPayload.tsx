...
export const addScopesForEditDealPayload = (
  deal: any,
  payload: any,
  newDealData: any
): any[] => {
  const { removeMchList = [], removeLiamList = [] } = newDealData;
  const combinedRemoveList = [...removeMchList, ...removeLiamList].map(
    ({ value }) => value
  );
  ...
  result.forEach((scope) => {
    ...
    if (combinedRemoveList.includes(value)) return;
...
export const addPromoRestrictionsForEditDealPayload = (
  deal: any,
  payload: any,
  newDealData: ICreateDealFormState
): any => {
  if (deal?.generalDealInfo?.type === FREE_SHIPPING_DEAL_TYPE)
    return payload?.["promo_restrictions"];
  if (newDealData?.dealLevelOptions === "no")
    return { product: { mch: [], liam: [] } };
  ...
  const existingPriceApplicability =
    deal?.applicableProducts?.priceApplicability;
  const editedPromoRestrictions = payload["promo_restrictions"];
  if (
    editedPromoRestrictions &&
    Object.values(editedPromoRestrictions).length === 0
  ) {
    ...
    const editedPriceApplicability =
      editedPromoRestrictions?.["price_applicability"]?.value;
