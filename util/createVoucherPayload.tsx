...
const getRewardType = ({ dealDiscountTab, dollarOff, percentageOff, fixedPriceOff, customPercentageOff, voucherLevel, basketDealType, basketDiscount }: ICreateVoucherFormState) => {
    let rewardType = '$_OFF'
...
const getVoucherApplyType = (dealApplyType: string) => DEAL_APPLY_TYPE[dealApplyType]

const generateCreateVoucherPayload = (formData: ICreateVoucherFormState) => {
    const {
        externalVoucherCode,
        description,
        priority,
    ...
    if (voucherLevel === 'product') {
        payload["scopes"] = getScopeData(productsCollectionTab, fileLIAM, fileMCH, liam, mch)
    }
