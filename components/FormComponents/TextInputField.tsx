...
const TextInputField = ({
  ...
  const handleBlur = (event: FocusEvent<HTMLInputElement>) => {
    ...
    displayDollarFormat &&
      setValue(name, value ? parseFloat(value).toFixed(2) : value, {
        shouldDirty: true,
        shouldTouch: true,
        shouldValidate: true,
      });
    customHandleBlur && customHandleBlur(value);
    if (!customHandleBlur && !displayDollarFormat) onBlur();
  };
  ...
  return (
    <div className={classNames.join(" ")}>
      ...
      <OutlinedInput
        ...
        endAdornment={
          <InputAdornment position="end">
            {endAdornment}
            {error && (
              <ErrorOutlineOutlinedIcon className={styles["error-icon"]} />
            )}
          </InputAdornment>
        }
        error={Boolean(error)}
      ...
      />
      {error && (
        <FieldErrorMessage message={error.message} testId={errorDataTestId} />
      )}
    </div>
  );
};
