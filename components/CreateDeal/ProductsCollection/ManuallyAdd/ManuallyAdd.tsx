...
function ManuallyAdd({ mchValue, liamValue }: any) {
  ...
  const deleteMCH = (index: number) => {
    mch.splice(index, 1);
    clearErrors(mch.length === 0 ? mchValue : `${mchValue}.${index}`);
    setValue(mchValue, mch, { shouldValidate: true });
  };
  ...
  return (
    <Grid>
      <div className={styles["title-container"]}>
        <Typography data-testid="mchTitle" className={commonStyles.mchHeading}>
          MCH(s)
        </Typography>
        <CustomTooltip
          descriptionKey="MCH"
      ...
      <Grid container mb={2}>
        {mch
          ?.slice(0, showAllListMCH ? mch.length : 5)
          .map((data: any, index: any) => {
            return (
              ...
              >
                ...
                <Grid>
                  <CloseIcon
                    data-testid="deleteMCH"
                    onClick={() => deleteMCH(index)}
        ...
        {showAllBtnMCH ? (
          <Grid mt={2} container display="grid" justifyContent="center">
            {showAllListMCH ? (
              ...
              <Grid item lg={12}>
                <Typography>
                  Showing <strong> 5 of {mch.length} </strong> recently added
                </Typography>
                <Grid
                  data-testid="showAllMCH"
                  onClick={handleShowAllMCH}
                  display="flex"
                  ml={5}
                >
