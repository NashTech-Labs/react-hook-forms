    ...
    <>
      <Grid container justifyContent="center" sx={{ marginTop: "32px" }}>
        <Grid item lg={6} md={8} sm={9}>
          <Typography variant="h4" className={summaryStyles.title}>
            {title}
          </Typography>
          <Typography mt={2} className={summaryStyles["sub-title"]}>
            Draft created on{" "}
            {moment(draftCreatedTimestamp).format(
              "MMMM D, YYYY [at] h:mm A z [EST]"
            )}
          </Typography>
          <Chip
            label="Draft"
            className={summaryStyles.Chip}
            sx={{ backgroundColor: "#666B73", marginBottom: "16px" }}
          />
        </Grid>
      </Grid>
      {Object.keys(config)
        .filter((step) => !excludeSteps.includes(step))
        .map((stepTitle: string) => {
          const basketLevelTitle =
            dealLevel === "basket" ? "Product Applicability" : stepTitle;
          const computedTitle =
            stepTitle === "Exclusions" ? basketLevelTitle : stepTitle;
          return (
            <Card
              className={commonStyles["step-card-container-summary"]}
              key={stepTitle}
            >
              ...
              {config[stepTitle].map(({ title, getValue, shouldHide }) => {
                ...
                return (
                  <>
                    <Typography
                      variant="h4"
                      className={summaryStyles.heading}
                      mt={4}
                    >
                      {title}
                    </Typography>
                    ...
                    title === "Customer preview" ? (
                      <>
                        {" "}
                        {customerPreview.map((data: any) => {
                          return (
                            <>
                              {" "}
                              <Typography className={summaryStyles.content}>
                                {data}
                              </Typography>{" "}
                            </>
                          );
